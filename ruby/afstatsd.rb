...
class Statsd
  ...
  def count(stat, count, sample_rate=1, note="")
	if sample_rate == 1 or rand < sample_rate
		send_metric CMetric.new(expand_name(stat), count, sample_rate, note)
	end
  end
  ...
  def gauge(stat, value, note="")
    send_metric GMetric.new(expand_name(stat), value, note)
  end
  ...
  def set(stat, value, note="")
    send_metric SMetric.new(expand_name(stat), value, note)
  end
  ...
  def timing(stat, ms, sample_rate=1, note="")
    if sample_rate == 1 or rand < sample_rate
		send_metric TMetric.new(expand_name(stat), ms, sample_rate, note)
	end	
  end
